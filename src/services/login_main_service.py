...
class LoginMainService(LoginMainServiceABC):
    ...
    def ejecutar_ingreso(self, correo_electronico: str, contrasena: str) -> tuple[Optional[Any], str]:
        profile = self.autenticacion_service.verificar_credenciales(correo_electronico, contrasena)
        if not profile:
            return None, ""

        user = self.usuario_repo.buscar_por_username(correo_electronico)
        role = user.rol if user else ""
        return profile, role
